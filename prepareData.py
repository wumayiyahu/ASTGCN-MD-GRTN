...
def search_data(sequence_length, num_of_depend, label_start_idx,
    ...
    x_idx = []
    for i in range(1, num_of_depend + 1):  #窗口
...
def get_sample_indices(data_sequence, num_of_weeks, num_of_days, num_of_hours,
    ...
    if num_of_hours > 0:  #得到hour_sample: (num_of_hours * points_per_hour, N, F)
    ...
    target = data_sequence[label_start_idx: label_start_idx + num_for_predict]   #target: (T_pre, N, F)
...
def read_and_generate_dataset(graph_signal_matrix_filename,
    ...
    '''
    data_seq = np.load(graph_signal_matrix_filename)['data']  # (sequence_length, num_of_vertices, num_of_features)->(T,N,F)
